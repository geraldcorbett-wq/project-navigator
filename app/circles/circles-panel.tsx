...
type Circle = { id: string; name: string; description: string | null; is_pinned: boolean; circle_members: Member[]; access: "owner" | "respond" };

function CircleRespond({circleId,token}:{circleId:string;token:()=>Promise<string|undefined>}) {
  const [content,setContent]=useState(""); const [message,setMessage]=useState("");
  async function send(event:FormEvent){event.preventDefault();const accessToken=await token();if(!accessToken)return;const response=await fetch(`/api/circles/${circleId}/responses`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${accessToken}`},body:JSON.stringify({content})});if(response.ok){setContent("");setMessage("Sent.");}else setMessage("Could not send.");}
  return <form className="featureForm" onSubmit={send}><label>Respond<textarea value={content} onChange={e=>setContent(e.target.value)} maxLength={4000} rows={2}/></label><button className="quietButton" disabled={!content.trim()}>Send</button>{message&&<small>{message}</small>}</form>;
}
...
export default function CirclesPanel() {
  ...
  return <div className="featureStack">
    ...
    <div className="itemList">{circles.map(circle=>selectionMode ? <button type="button" className={`itemCard selectionCard ${circle.is_pinned?"pinnedCard":""}`} key={circle.id} onClick={()=>void selectEntity("circle",circle.id)}><div><h2>{circle.name}</h2>{circle.description&&<p>{circle.description}</p>}<small>{circle.circle_members?.length||0} member(s)</small></div></button> : <article className={`itemCard ${circle.is_pinned?"memoryPinned pinnedCard":""}`} key={circle.id}><div>{circle.is_pinned&&<span className="pinnedBadge">Pinned</span>}<h2>{circle.name}</h2>{circle.description&&<p>{circle.description}</p>}<small>{circle.circle_members?.length||0} member(s){circle.is_pinned?" · pinned":""}</small></div>{circle.access === "owner" ? <><div className="cardActions"><button className="textLink" type="button" onClick={()=>void patch(circle.id,{is_pinned:!circle.is_pinned})}>{circle.is_pinned?"unpin":"pin"}</button><button className="dangerLink" type="button" onClick={()=>void remove(circle.id)}>delete</button></div><RelatedPanel entityType="circle" entityId={circle.id}/></> : <CircleRespond circleId={circle.id} token={token} />}</article>)}</div>
  </div>;
}
