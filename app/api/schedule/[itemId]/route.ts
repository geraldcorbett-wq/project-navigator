...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ itemId: string }> }) {
  ...
  if ("is_pinned" in body) {
    ...
    if (nextPinned) {
      const { count, error: countError } = await auth.supabase.from("schedule_items").select("id", { count: "exact", head: true }).eq("user_id", auth.user.id).eq("is_pinned", true).neq("id", itemId);
  ...
  const { data, error } = await auth.supabase.from("schedule_items").update(update).eq("id", itemId).eq("user_id", auth.user.id).select("*").single();
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ itemId: string }> }) {
  ...
  try {
    await deleteEntityLinks(auth.supabase, auth.user.id, "schedule", itemId);
  } catch (error) {
    return jsonError(error instanceof Error ? error.message : "Could not remove connections.", 500);
  }
  const { error } = await auth.supabase.from("schedule_items").delete().eq("id", itemId).eq("user_id", auth.user.id);
