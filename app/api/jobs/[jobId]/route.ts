...
export async function PATCH(request: NextRequest, { params }: Context) {
  ...
  if (["completed", "failed", "cancelled"].includes(String(body.status))) patch.completed_at = new Date().toISOString();
  const { data, error } = await auth.supabase.from("background_jobs").update(patch)
    .eq("id", jobId).eq("user_id", auth.user.id).select("*").maybeSingle();
...
export async function DELETE(request: NextRequest, { params }: Context) {
  ...
  const auth = await authenticateRequest(request);
  if (isAuthFailure(auth)) return jsonError(auth.error, auth.status);
  const { data, error } = await auth.supabase.from("background_jobs").delete()
    .eq("id", jobId).eq("user_id", auth.user.id).select("id").maybeSingle();
  if (error) return jsonError(error.message, 500);
  if (!data) return jsonError("Job not found.", 404);
