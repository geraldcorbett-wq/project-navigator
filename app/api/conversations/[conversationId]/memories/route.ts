...
export async function POST(request: NextRequest, { params }: { params: Promise<{ conversationId: string }> }) {
  ...
  const [{ data: conversation }, { data: memory }] = await Promise.all([
    auth.supabase.from("conversations").select("id").eq("id", conversationId).eq("user_id", auth.user.id).maybeSingle(),
  ...
  const { error } = await auth.supabase.from("memory_conversations").upsert({ memory_id: memoryId, conversation_id: conversationId, user_id: auth.user.id }, { onConflict: "memory_id,conversation_id" });
  if (error) return jsonError(error.message, 500);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ conversationId: string }> }) {
  ...
  if (!memoryId) return jsonError("memory_id is required.", 400);
  const { error } = await auth.supabase.from("memory_conversations").delete().eq("memory_id", memoryId).eq("conversation_id", conversationId).eq("user_id", auth.user.id);
  if (error) return jsonError(error.message, 500);
